...
#include "config.h"
	...
	char key = 0,dis_key = 0;
	char temputer_H=12,temputer_L =34;
...
//  mDelay(1);
    ...
    {  
			...
			temputer_H = Rxd_data ;temputer_L = Rxd_data1  ;		
				//key=2;
			}
			
			if (dis_key)//dis_key 为显示按键还是温度的标志位
			{//=0 则是显示温度		
				 DisBuff[2] = 12;    
				 DisBuff[3] = temputer_L/10; 
				 DisBuff[0] =temputer_H%10;	   
				 DisBuff[1] = temputer_H/10; 
			}else
			{
				 DisBuff[2] = 0;    
				 DisBuff[3] = 0; 
...
if (dis_key) number = (0x7F)&LED_Disp[DisBuff[0]] ;
	else number = LED_Disp[DisBuff[0]];
